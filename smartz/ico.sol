...
contract ICO is TimedCrowdsale, 
    ...
    uint constant public cSoftCap = {{ softcap }};

    RefundEscrow public mEscrow;

    {{ _if(min_contribution, "uint constant  MIN_CONTRIBUTION = " + str(min_contribution) + ";") }}
   ...
   _if(min_contribution, """
    ...
    function processRemainingTokens() internal {
        ...
        if (currentBalance != 0) {
            ...
            {{ _if(need_burn, "BurnableToken(address(token)).burn(currentBalance);") }}
            {{ _if(not need_burn, "token.transfer(mEscrow.beneficiary(), currentBalance);") }}
        } 
    ...
    function needBurn() internal pure returns (bool) {
        return  {{ "true" if need_burn else "false" }};
    }
